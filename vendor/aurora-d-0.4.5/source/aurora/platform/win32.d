        ...
        private LRESULT processMessage(UINT message, WPARAM wParam, LPARAM lParam)
        {
            ...
            switch (message)
            {
                ...
                case WM_NCACTIVATE:
                    ...
                    break;
                case WM_NCHITTEST:
                    // A fullscreen surface is not edge-resizable and its client
                    // area reaches the screen edges. DefWindowProc made the
                    // right edge report HTVSCROLL (WS_VSCROLL is set for scroll
                    // integration) and the bottom/right the resize border, which
                    // stole every click from the taskbar's edge-anchored
